/* EDF-based scheduling policy for SCHED */
#include "sched.h"
#include "schedproc.h"
#include <assert.h>
#include <minix/com.h>
...
#define DEFAULT_DEADLINE        1000  // Default deadline in ms or ticks
...
int do_noquantum(message *m_ptr)
{
    ...
    for (int i = 0; i < NR_PROCS; ++i) {
        if (schedproc[i].flags & IN_USE) {
            if (!selected || schedproc[i].deadline < selected->deadline) {
...
int do_start_scheduling(message *m_ptr)
{
    ...
    // Assign a simulated deadline (for now)
    rmp->deadline = DEFAULT_DEADLINE - proc_nr_n * 50;  // e.g., earlier for lower proc_nr
